...
namespace qr_code
{
    struct Preprocess
    {
        Preprocess(image::rgb24_image image, image::rgb24_image equalized,
                   image::gray8_image gray, image::gray8_image bin,
                   image::gray8_image denoises)
            : image_originel(image)
            , image_equalized(equalized)
            , image_grayscale(gray)
            , image_binary(bin)
            , image_denoise(denoises) {};
