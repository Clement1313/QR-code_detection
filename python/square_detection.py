...
def draw_qr(image, qr_corners):
    """
    :param image: image étudié
    :param qr_corners:  coordonnée du qr code
    :return:  image avec un le qr code entouré en rouge
    """
    img = image.copy()
    if img.shape[2] == 4:
        img = img[:, :, :3]
...
def get_inner_square(square):
    """
    :param square:  image de la région
    :return: image de la région sans les éléments qu'ils y a autour de l'élément au centre de la région
    """
...
def get_corner(im:np.ndarray):
    """
    :param im: une région de l'image
    :return: les keypoints (edges) du carré au centre de la région (dans le cas où on a un marqueur de position)
    """
...
def get_four_coord(coords):
    """
    :param coords: liste des keypoints (les edges) détecté d'une région
    :return: renvoie les 4 keypoints qui ont le plus la forme d'un quadrilatère
    """
    if coords.shape[0] < 4:
        return coords
    ...
    if coords.shape[0] > 4:
        try:
            hull = ski.morphology.convex_hull_image
        except Exception:
            pass
...
#=============================================================================
#               VEFICATION POUR MARQUEUR DE POSITION
#=============================================================================
def check_longeur(coords:np.ndarray)->bool:
    """
    :param coords:  keypoints (edge) du carré au centre d'un marqueur de position
    :return: vérifie que les keypoints sont à la meme distance
    """
    lengths = []
    for i in range(4):
        p1 = coords[i]
...
def is_convex_quad(coords):
    """
    :param coords: keypoints (edge) du marqueur de position
    :return: vérfie que les keypoints forme bien un élément convex
    """
    n = 4
    cross_signs = []
    for i in range(n):
...
def check_region_shape(region, min_solidity=0.7, max_eccentricity=0.6, min_axis_ratio=0.5):
    return not (region.eccentricity > max_eccentricity)


#=============================================================================
#               PARTIE DETECTION MARQUEUR DE POSITION
#=============================================================================
def square_filter(denoise:np.ndarray,binary:np.ndarray, region: object)->(bool,np.ndarray):
    """
    :param denoise: image denoise
    :param binary: image binaire
    :param region:  region de l'image
    :return: renvoies si une région pourrait être un marqueur de position
    """
    if region.area < 50:
...
#=============================================================================
#               METHODE DE COMPARAISON DES TRIPLETS
#=============================================================================
...
#=============================================================================
#               RECUPERATION DES TRIPLETS SELON UN SCORE
#=============================================================================
...
def get_triplets(elements,angle_tolerance=35):
    """
    :param elements: différentes régions pouvant être un marqueur de position
    :param angle_tolerance: tolerance de l'angle entre trois régions
    :return: renvoie une première version des triplets des marqueurs de positions
    """
    candidates = []
...
def filter_contained_triplets(triplets):
    """
    :param triplets: triplets trouvé après un premier filtrage
    :return: second filtre pour enlever les triplets dont les keypoints sont dans un qr code
    """
